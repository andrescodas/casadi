...
void SDPSDQPInternal::init(){
  ...
  MX znp = DMatrix(00,00,00,n_+1,n_+1);
  for (int k=0;k<n_;++k) {
    MX gk = horzcat(g_socp(k,ALL),DMatrix(00,00,00,1,1));
    MX fk = -blockcat(znp,gk,trans(gk),DMatrix(00,00,00,1,1));
  ...
  fi.push_back(blkdiag(DMatrix(00,00,00,f_sdqp.size1(),f_sdqp.size1()),-fin));
  
  MX h0 = horzcat(h_socp,DMatrix(00,00,00,1,1));
