...
void SOCPQCQPInternal::init(){
  ...
  for (int i=0;i<nq_+1;++i) {
    ...
    socp_g.push_back(blkdiag(cholesky_[i].getFactorizationSparsity(true),sp_denseQQQ(1,1)));
  }

  // Create an socpsolver instance
  SOCPSolverCreator socpsolver_creator = getOption("socp_solver");
  socpsolver_ = socpsolver_creator(socpStruct("g",horzcat(socp_g),"a",vertcat(input(QCQP_SOLVER_A).sparsity(),sp_sparseQQQ(1,nc_))));
