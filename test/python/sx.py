...
class SXtests(casadiTestCase):
  ...
  def test_subsassignment(self):
    ...
    s=y.sparsity()
    for k in kl:
      z[s.getCol()[k],s.row()[k]]=1.0
    self.checkarray(y,z,"nonzero scalar assignment")
    y[kl]=DMatrix(kl)
    
    cnt=0
    for k in kl:
      z[s.getCol()[k],s.row()[k]]=kl[cnt]
      cnt+=1
  ...
  def test_truth(self):
    ...
    self.assertTrue(bool(SX(-0.2)))
    self.assertRaises(Exception, lambda : bool(ssymQQQ("x")))
    self.assertRaises(Exception, lambda : bool(ssymQQQ("x")>0))
  ...
  def test_jacobian_empty(self):
    ...
    s = jacobian(x,ssymQQQ("x",4,0)).shape
