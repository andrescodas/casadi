...
class typemaptests(casadiTestCase):
  ...
  def test_1(self):
    ...
    arrays = [DMatrix(00,00,00,4,3,[0,2,2,3],[1,2,1],[3,2.3,8])]
    for i in range(len(arrays)):
      m = arrays[i]
      zt=trans(trans(m))
      self.assertTrue(isinstance(zt,DMatrix),"DMatrix expected")
  ...
  def test_setget(self):
    ...
    dm=DMatrix(00,00,00,4,3,[0,2,2,3],[1,2,1],[3,2.3,8])
  ...
  def test_conversion(self):
    ...
    w = DMatrix(00,00,00,4,3,[0,2,2,3],[1,2,1],[3,2.3,8])
    d = array([[1,2,3],[4,5,6]])
  ...
  def test_autoconversionMX(self):
    ...
    ## MX & MX
    for s in [MX(00,00,00,"x"),MX(00,00,00,"x",2,2)]:
      for z in [MX(00,00,00,"x"),MX(00,00,00,"x",2,2)]:
        print "z = %s, s = %s" % (str(z),str(s))
        print "  z = %s, s = %s" % (type(z),type(s))
        tests(z,s)
        
    for (s,x,y) in [
                  (matrix([[1,2],[3,4]]),ssym("x",2,2),MX(00,00,00,"x",2,2))    
                  ]:
  ...
  def test_issue(self):
    ...
    DMatrix(00,00,00,4,3,(0,2,2,3),[1,2,1],[0.738,0.39,0.99])
    DMatrix(00,00,00,4,3,list(a),[1,2,1],[0.738,0.39,0.99])
    DMatrix(00,00,00,4,3,a,[1,2,1],[0.738,0.39,0.99])
    DMatrix(00,00,00,4,3,[0,2,2,3],[1,2,1],(0.738,0.39,0.99))
    DMatrix(00,00,00,4,3,[0,2,2,3],[1,2,1],list(b))
    DMatrix(00,00,00,4,3,[0,2,2,3],[1,2,1],b)
  ...
  def test_imatrix(self):
    ...
    self.checkarray(array(B),DMatrix(00,00,00,2,2,2),"Imatrix")
