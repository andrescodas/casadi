...
class MXtests(casadiTestCase):
  ...
  def test_MX_shapes(self):
      ...
      for sp in [sp_dense(0,0),sp_dense(0,2),sp_dense(2,0),sp_dense(1,1),sp_dense(2,2), CCSSparsity(4,3,[0,2,2,3],[1,2,1])]:
        for v in [0,1,0.2]:
          x_ = DMatrix(sp,v)
      ...
      for sp in [sp_sparse(1,1),sp_dense(1,1),sp_sparse(3,4),sp_dense(3,4), CCSSparsity(4,3,[0,2,2,3],[1,2,1])]:
        for v1 in [0,1,0.2,-0.2]:
          ...
          x1s=xx1s[sp]
          for sp2 in [sp_sparse(1,1),sp_dense(1,1),sp_sparse(3,4),sp_dense(3,4), CCSSparsity(4,3,[0,2,2,3],[1,2,1])]:
  ...
  def test_MXConstant(self):
      ...
      for sp in [sp_dense(1,1),sp_sparse(1,1),sp_sparse(3,4),sp_dense(3,4), CCSSparsity(4,3,[0,2,2,3],[1,2,1])]:
        for v1 in [0,1,0.2,-0.2]:
          x1_ = DMatrix(sp,v1)
          x1=MX(sp,v1)
          for sp2 in [sp_dense(1,1),sp_sparse(1,1),sp_sparse(3,4),sp_dense(3,4), CCSSparsity(4,3,[0,2,2,3],[1,2,1])]:
            for v2 in [0,1,0.2,-0.2]:
              x2_ = DMatrix(sp2,v2)
              x2=MX(sp2,v2)
              for (casadiop, numpyop,name, flags) in self.matrixbinarypool.zip():
