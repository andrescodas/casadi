...
class Toolstests(casadiTestCase):
  ...
  def test_structure(self):
    ...
    self.checkarray(init['X',0,-1,'p',vertcat,:],DMatrix.ones(6,9)*7)

    with self.assertRaises(Exception):
      init['X',0,-1,'p',:] = [1,2,3,4,5,6]
      
    init['X',0,-1,'p',:] = repeated([1,2,3,4,5,6])
    self.checkarray(init['X',0,-1,'p',horzcat,:,2],DMatrix.ones(9)*3)
    ...
    init['X',0,-1,'p',vertcat,:] = DMatrix.ones(6,9)*2
    
    self.assertEqual(sum(init.cat==2),6*9)

    init['X',0,-1,'p',horzcat,:,2] = DMatrix.ones(9)*3
    
    self.assertEqual(sum(init.cat==2),6*9-9)
    self.assertEqual(sum(init.cat==3),9)

    self.checkarray(init['X',0,-1,'p',vertcat,:,[2,3]],horzcat([DMatrix.ones(1,9)*3,DMatrix.ones(1,9)*2]))
    
    init['X',:,-1,'p',vertcat,:] = repeated(DMatrix.ones(6,9)*2)
  ...
  def test_structure_repeated_dmatrix(self):
    ...
    self.checkarray(d,vertcat([range(12),DMatrix.zeros(12),DMatrix.zeros(12)]))
  ...
  def test_bug_structSXMX(self):
    ...
    X_sx["S"] = DMatrix(00,00,00,range(n,n+n*n),n,n)
   
    X_mx = struct_MX(x_sx)
    X_mx["x"] = DMatrix(range(n))
    X_mx["S"] = DMatrix(00,00,00,range(n,n+n*n),n,n)
    
    self.checkarray(x_sx.struct.map[("S",)],DMatrix(00,00,00,range(n,n+n*n),n,n))
    self.checkarray(x_mx.struct.map[("S",)],DMatrix(00,00,00,range(n,n+n*n),n,n))
    ...
    for s, S in [(x_sx,struct_ssym),(x_mx,struct_msym)]:
      ...
      hX["w","S"] = DMatrix(00,00,00,range(n,n+n*n),n,n) 
      
      self.checkarray(h.struct.map[("w","S",)],DMatrix(00,00,00,range(n,n+n*n),n,n))
    ...
    X_sx = struct_SX(x_sx)
    X_sx["x"] = DMatrix(range(n))
    X_sx["S"] = DMatrix(00,00,00,range(n,n+n*m),m,n)
   
    X_mx = struct_MX(x_sx)
    X_mx["x"] = DMatrix(range(n))
    X_mx["S"] = DMatrix(00,00,00,range(n,n+n*m),m,n)
    
    self.checkarray(x_sx.struct.map[("S",)],DMatrix(00,00,00,range(n,n+n*m),m,n))
    self.checkarray(x_mx.struct.map[("S",)],DMatrix(00,00,00,range(n,n+n*m),m,n))
