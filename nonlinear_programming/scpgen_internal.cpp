  ...
  void SCPgenInternal::init(){
    ...
    if(gauss_newton_){    
      ...
      for(vector<Var>::iterator it=v_.begin(); it!=v_.end(); ++it){
        ss.str(string());
        ss << "lam_x" << i++;
        it->v_lam = msymQQQ(ss.str(),it->v.sparsity());
      }
    
      // Lagrange multipliers for the nonlinear constraints
      g_lam = msymQQQ("g_lam",1,ng_);
    ...
    for(vector<Var>::iterator it=v_.begin(); it!=v_.end(); ++it){
      ss.str(string());
      ss << "d" << i++;
      it->d = msymQQQ(ss.str(),it->v.sparsity());
    ...
    if(!gauss_newton_){
      ...
      for(vector<Var>::iterator it=v_.begin(); it!=v_.end(); ++it){
        ss.str(string());
        ss << "d_lam" << i++;
        it->d_lam = msymQQQ(ss.str(),it->v.sparsity());
    ...
    MX du = msymQQQ("du",1,nx_);   // Step in u
    MX g_dlam;               // Step lambda_g
    if(!gauss_newton_){
      g_dlam = msymQQQ("g_dlam",g_lam.sparsity());
    }
