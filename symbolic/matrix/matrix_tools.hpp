...
namespace CasADi{
  ...
  template<class T>
  Matrix<T> reshape(const Matrix<T>& a, int n, int m){
    CCSSparsity sp = a.sparsity().reshape(m,n);
  ...
  template<class T>
  Matrix<T> sumCols(const Matrix<T> &x) {
    return mul(Matrix<T>::ones(x.size2(),1),x);
  }

  template<class T>
  Matrix<T> sumRows(const Matrix<T> &x) {
    return mul(x,Matrix<T>::ones(1,x.size1()));
  }
  ...
  void qr(const Matrix<T>& A, Matrix<T>& Q, Matrix<T> &R){
    ...
    for(int i=0; i<n; ++i){
      ...
      for(int j=0; j<i; ++j){
        ...
        ri(j,0) = mul(qj,trans(qi)); // Modified Gram-Schmidt
  ...
  Matrix<T> nullspace(const Matrix<T>& A) {
    ...
    for (int i=0;i<n;++i) {
      ...
      X(range(i,m),range(i,n))-= beta*mul(mul(X(range(i,m),range(i,n)),trans(u)),u);
      us.push_back(u);
      betas.push_back(beta);
    }
    
    for (int i=n-1;i>=0;--i) {
      seed(range(m-n),range(i,m)) -= betas[i]*mul(trans(us[i]),mul(us[i],seed(range(m-n),range(i,m))));
    }
