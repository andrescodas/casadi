...
namespace CasADi{
  ...
  void SymbolicQRInternal::init(){
    ...
    SXMatrix Aperm(00,00,00,A.size1(),0);
    ...
    SXMatrix bperm(00,00,00,b.size1(),0);
    ...
    SXMatrix x(00,00,00,xperm.size1(),0);
    ...
    // Permute the right hand side
    bperm = SXMatrix(00,00,00,b.size1(),0);
    for(int i=0; i<b.size2(); ++i){
      bperm.resize(i+1,b.size1());
    ...
    // Permute back the solution
    x = SXMatrix(00,00,00,xperm.size1(),0);
    for(int i=0; i<xperm.size2(); ++i){
      x.resize(i+1,xperm.size1());
