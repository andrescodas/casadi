...
Matrix<double> numSample1D(FX &fx, const Matrix<double> &grid) {
  ...
  casadi_assert_message(fx.input().size2()==1, "numSample1D:: supplied fx must have a row-matrix-like first input, but you supplied a shape " << fx.input().dimString() << ".");
  casadi_assert_message(fx.input().dense()==1, "numSample1D:: supplied fx must have dense input, but you supplied " << fx.input().dimString() << ".");
  casadi_assert_message(grid.size1()==fx.input().size1(), "numSample1D:: supplied grid has a shape " << grid.dimString() << ", but the row size does not match the row size of the supplied fx first input, which is " << fx.input().dimString() << ".");
