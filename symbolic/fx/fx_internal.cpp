      ...
      for(int csd=0; csd<D.size2(); ++csd) {
        ...
        while(!f_finished) {
          ...
          if (bvec_i==bvec_size || csd==D.size2()-1) {
            ...
            IMatrix lookup = IMatrix::sparseQQQ(lookup_row,lookup_col,lookup_value,bvec_size,coarse.size());
            ...
            IMatrix duplicates = IMatrix::sparseQQQ(lookup_row,lookup_col,lookup_value,bvec_size,coarse.size()) - lookup;
            makeSparse(duplicates);
            SubMatrix<Matrix<int>,CCSSparsity,int> temp(lookup,duplicates.sparsity(),0); // NOTE: Not intended use of SubMatrix
            temp = -bvec_size;
      ...
      for(int csd=0; csd<D.size2(); ++csd) {
        ...
        while(!f_finished) {
          ...
          if (bvec_i==bvec_size || csd==D.size2()-1) {
            ...
            // Construct lookup table
            IMatrix lookup = IMatrix::sparseQQQ(lookup_row,lookup_col,lookup_value,bvec_size,coarse_col.size());
