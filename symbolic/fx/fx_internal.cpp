...
namespace CasADi{
  ...
  CCSSparsity FXInternal::getJacSparsityPlain(int iind, int oind){
    ...
    CCSSparsity ret = sp_tripletQQQ(nz_in,nz_out, use_fwd ? jrow : jcol, use_fwd ? jcol : jrow);
  ...
  CCSSparsity FXInternal::getJacSparsityHierarchicalSymm(int iind, int oind){
    ...
    while (!hasrun || coarse.size()!=nz+1) {
      ...
      for(int csd=0; csd<D.size2(); ++csd) {
        ...
        while(!f_finished) {
          ...
          if (bvec_i==bvec_size || csd==D.size2()-1) {
            ...
            IMatrix lookup = IMatrix::sparse(lookup_col,lookup_row,lookup_value,coarse.size(),bvec_size);
            ...
            IMatrix duplicates = IMatrix::sparse(lookup_col,lookup_row,lookup_value,coarse.size(),bvec_size) - lookup;
            makeSparse(duplicates);
            SubMatrix<Matrix<int>,CCSSparsity,int> temp(lookup,duplicates.sparsity(),0);
            temp = -bvec_size;
      ...
      r = sp_tripletQQQ(fine.size()-1, fine.size()-1, jrow, jcol);
      coarse = fine;
      hasrun = true;
    }
  ...
  CCSSparsity FXInternal::getJacSparsityHierarchical(int iind, int oind){
    ...
    while (!hasrun || coarse_col.size()!=nz_out+1 || coarse_row.size()!=nz_in+1) {
      ...
      CCSSparsity rT = r.transpose();
      
      /**       Decide which ad_mode to take           */
      
      // Forward mode
      CCSSparsity D1 = rT.unidirectionalColoring(r);
      // Adjoint mode
      ...
      for(int csd=0; csd<D.size2(); ++csd) {
        ...
        while(!f_finished) {
          ...
          if (bvec_i==bvec_size || csd==D.size2()-1) {
            ...
            // Construct lookup table
            IMatrix lookup = IMatrix::sparse(lookup_col,lookup_row,lookup_value,coarse_col.size(),bvec_size);
      ...
      if (use_fwd) {
        // Construct fine sparsity pattern
        r = sp_tripletQQQ(fine_row.size()-1, fine_col.size()-1, jrow, jcol);
        coarse_col = fine_col;
        coarse_row = fine_row;
      } else {
        // Construct fine sparsity pattern
        r = sp_tripletQQQ(fine_col.size()-1, fine_row.size()-1, jcol, jrow);
        coarse_col = fine_row;
        coarse_row = fine_col;
      }
      hasrun = true;
  ...
  void FXInternal::getPartition(int iind, int oind, CCSSparsity& D1, CCSSparsity& D2, bool compact, bool symmetric){
  ...
  void FXInternal::generateIO(CodeGenerator& gen){
    ...
    for(int i=0; i<n_io; ++i){
      ...
      const CCSSparsity& sp = i<n_i ? input(i).sparsity() : output(i-n_i).sparsity();
