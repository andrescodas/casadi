...
void OCPSolverInternal::init(){
  ...
  input(OCP_LBX) = input(OCP_UBX) = input(OCP_X_INIT) = DMatrix::zeros(nk_+1,nx_);
  input(OCP_LBU) = input(OCP_UBU) = input(OCP_U_INIT) = DMatrix::zeros(nk_,nu_);
  input(OCP_LBP) = input(OCP_UBP) = input(OCP_P_INIT) = DMatrix::zeros(1,np_);
  input(OCP_LBH) = input(OCP_UBH) = DMatrix::zeros(nk_+1,nh_);
  input(OCP_LBG) = input(OCP_UBG) = DMatrix::zeros(1,ng_);
