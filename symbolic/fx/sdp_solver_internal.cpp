...
void SDPSolverInternal::init() {
  ...
  for (int i=0;i<nb_;++i) {
    full_blocks.push_back(ssymQQQ("block",block_sizes_[i],block_sizes_[i]));
  }
  ...
  if (nb_>0) {
    ...
    SXMatrix G = ssymQQQ("G",input(SDP_SOLVER_G).sparsity());
    SXMatrix F = ssymQQQ("F",input(SDP_SOLVER_F).sparsity());
