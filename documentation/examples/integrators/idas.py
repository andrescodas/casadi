...
ode = horzcat([u,lambd*x,v,lambd*y+g])
alg = x**2+y**2-L**2
...
ode = horzcat([u,lambd*x])
alg = horzcat([x**2+y**2-L**2, u*x+v*y,u**2-g*y+v**2+L**2*lambd])
x_all = horzcat([x,u])
z_all = horzcat([y,v,lambd])
f = SXFunction(daeIn(x=x_all,z=z_all,p=p_all,t=t),daeOut(ode=ode,alg=alg))
f.init()
