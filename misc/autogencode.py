...
""" % (self.enum,self.enum,self.enum)
    ...
    #  s+="typedef %sIOSchemeVector<CCSSparsity> %sure;\n" % (self.enum,self.enum)
...
"""  % (self.enum,self.enum,", ".join([name for name, doc, enum in self.entries]))
  ...
  def swigcode(self):
    ...
    if self.enum.endswith('Struct'):
      s+="%template(" + self.name + ") " + self.name + "<CasADi::CCSSparsity>;\n"
    else:
      ...
      s+="%template(" +  "IOSchemeVector" + self.enum + "CCSSparsity) " + self.enum + "IOSchemeVector<CCSSparsity>;\n"
      #s+="%rename(" + self.name + ") " + self.name + "<CasADi::SXMatrix>;\n"
      #s+="%rename(" + self.name + ") " + self.name + "<CasADi::MX>;\n"
      #s+="%rename(" + self.name + ") " + self.name + "<CasADi::CCSSparsity>;\n"
      ...
      s+="%rename(" + "IOSchemeVector" + self.enum + ") " + "IOSchemeVector" + self.enum+ "CCSSparsity;\n"
    s+="}\n"
    return s
  ...
  def pureswigcode(self):
    s="namespace CasADi {\n"
    if self.enum.endswith('Struct'):
      s+="%template(" + self.enum + "ure) " + self.enum + "IOSchemeVector<CCSSparsity>;\n"
    s+="}\n"
    return s
...
class IOSchemeVector;class CCSSparsity;\n""")
...
def IOSchemeVector(arg,io_scheme):
  ...
  try:
    ...
    return IOSchemeVectorCCSSparsity(arg,io_scheme)
  except:
    pass
  raise Exception("IOSchemeVector called with faulty arguments. Individual values must be SXMatrix, MX or CCSSparsity.")
